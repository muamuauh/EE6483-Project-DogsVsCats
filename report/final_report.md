...
Figure 1 shows the final Dogs vs Cats architecture used for the submitted model.
...
**Figure 1. Final Dogs vs Cats ResNet34 architecture.**
...
The pretrained ResNet34 backbone acts as the feature extractor. The original ImageNet classifier was replaced by a new fully connected layer with 2 output units. The two logits correspond to `cat` and `dog`, where the dataset label mapping is `cat = 0` and `dog = 1`. During training, the logits are passed to `CrossEntropyLoss`; during validation and test prediction, the class with the larger logit is selected as the predicted label.
...
**Figure 2. CIFAR-10 ResNet18 extension architecture.**
